...
آمار، تنظیمات، محصولات، کاربران، قیمت پلن‌ها و دفتر کل — همه با دکمه،
...
| فایل | نقش |
|---|---|
| `foxcoin.js` | هسته (نسخه ۱.۱.۰): محصول کوینی + ابزار مدیریت |
| `foxcoin-ui.js` | رابط کاربری کاربر عادی (فاز ۲) |
| `foxcoin-admin.js` | **پنل مدیریت (فاز ۳ — جدید)** |
| `patch-foxcoin-admin.py` | وصله اتصال پنل به `bot.js` (جدید) |
| `upgrade-foxcoin.py` | ارتقای هسته قدیمی سرور به نسخه کامل (نسخه ۲) |
...
| 🛍 محصولات | فعال/غیرفعال‌سازی، ویرایش قیمت کوینی، حذف با تأیید |
...
#    foxcoin.js (نسخه جدید)، foxcoin-admin.js
...
```bash
node foxcoin.js selftest     # ۳۱ تست هسته
node foxcoin-ui.js           # تست رابط کاربری
node foxcoin-admin.js        # ۳۶ تست پنل مدیریت
```
