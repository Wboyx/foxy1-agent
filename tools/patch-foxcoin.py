...
"""
════════════════════════════════════════════════════════════════
 PATCH FOXCOIN — افزودن فاکس کوین به ربات
 نسخه: 1.0 | 2026-08-21
════════════════════════════════════════════════════════════════

چه می‌کند:
  سه خط به bot.js اضافه می‌کند و بس.

چرا اسکریپت به‌جای ویرایش دستی:
  ویرایش دستی سه هزار و نهصد خط کد زنده، منبع اشتباه است.
  این اسکریپت قبل از هر کار لنگرگاه‌ها را می‌شمارد، اگر یکی از
  آن‌ها نبود یا بیش از یکی بود، اصلاً دست نمی‌زند.
...
استفاده:
  python3 patch-foxcoin.py            نمایش برنامه، بدون تغییر
  python3 patch-foxcoin.py --apply    اعمال واقعی
  python3 patch-foxcoin.py --revert   برگرداندن آخرین بکاپ همین وصله
"""
...
BOT = "/root/foxteam-bot/bot.js"
BACKUP_DIR = "/root/botjs-backups"
MARK = "foxcoin-ui"
...
STEPS = [
    {
        "name": "بارگذاری ماژول",
        "anchor": "async function tg(config, method, body) {",
        "where": "before",
        "add": "const coinUI = require('./foxcoin-ui');\n\n",
    },
    {
        "name": "دکمه در منوی اصلی",
        "anchor": '[{ text: L.btn_referral, callback_data: "referral" }],',
        "where": "after",
        "add": '\n    [{ text: coinUI.MENU_BUTTON.text, callback_data: "coin" }],',
    },
    {
        "name": "شرط در مسیریاب",
        "anchor": 'if (data === "ignore") return;',
        "where": "after",
        "add": (
...
def check_env():
    ...
    for m in ("foxcoin.js", "foxcoin-ui.js"):
        p = os.path.join(os.path.dirname(BOT), m)
        if not os.path.exists(p):
            problems.append("ماژول نصب نشده: " + p)
...
def main():
    ...
    if "--apply" not in args:
        ...
        print("   python3 patch-foxcoin.py --apply")
        return 0

    stamp = time.strftime("%Y-%m-%d-%H%M%S")
    bak = os.path.join(BACKUP_DIR, "bot.js.before-foxcoin-" + stamp)
    shutil.copy2(BOT, bak)
    print("\nبکاپ:", bak)

    new = apply_patch(src)
    # پسوند باید js بماند، وگرنه node --check فایل را نمی‌شناسد
    tmp = BOT + ".foxcoin-tmp.js"
    ...
    print("\nخط‌های اضافه‌شده: %d" % (len(new.splitlines()) - len(src.splitlines())))
    print("\nحالا سرویس را ری‌استارت کن:")
    print("   systemctl restart foxteam-bot")
    print("\nاگر چیزی خراب شد، برگشت:")
    print("   python3 patch-foxcoin.py --revert")
    return 0
