...
تغییرات نسخه ۱.۱:
  بلوک مسیریابی admin حالا بیرون از بلوک coin قرار می‌گیرد (همسطح
  آن). در نسخه ۱.۰ داخل بلوک coin می‌افتاد و چون شرط بیرونی برقرار
  نبود، دکمه مدیریت هیچ‌وقت کار نمی‌کرد. اگر نسخه ۱.۰ زده‌ای،
  فقط --revert بزن و دوباره --apply کن.
════════════════════════════════════════════════════════════════
...
STEPS = [
    ...
    },
    {
        ...
        # لنگرگاه شامل براکت بسته‌شونده بلوک coin است تا بلوک admin
        # بیرون از آن (همسطح) بنشیند. نسخه ۱.۰ آن را داخل بلوک coin
        # می‌گذاشت و دکمه مدیریت هرگز کار نمی‌کرد.
...
def main():
    ...
    if already:
        if ok:
            ...
            return 0
        print("\nوصله قبلاً خورده ولی ساختارش درست نیست.")
        print("این معمولاً یعنی نسخه ۱.۰ زده شده که بلوک مدیریت را داخل")
        print("بلوک coin می‌گذاشت و دکمه مدیریت هیچ‌وقت کار نمی‌کرد.")
        print("برگردان و دوباره اعمال کن:")
